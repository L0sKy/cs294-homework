---
name: act-on-plan
description: Implement the plan which is given.
---

# Developer Flow

This document outlines the flow both a human and an agentic AI developer shall follow to
add new features, fix bugs, and improve the codebase.
...
No matter if a TODO is given write a report in `docs/dones/DONE-xxx.md` where `xxx` is the same as `TODO-xxx`,
...
- If some of the test cases do not work, you can still commit it. However, you should comprehensively document it in the DONE document:
  ...
  5. When finalizing a TODO, you **CANNOT** bypass the `pre-commit` check using `--no-verify`.
  6. If not specified at the beginning of the given plan, there is NO PRE-EXISTING test failure. DO NOT use that as an excuse to bypass the tests when finalizing! You should make sure that your changes DO NOT break any thing!