...
Assuming you are an expert to this project after [read the document](./read-the-doc.mdc).

When you are asked to make a plan, such a plan should follow these rules:
...
0. The order of a plan matters:
   - Test-driven development is prefered. Thus, it is preferred to develop the test cases before the features.
      - However, DO NOT infinitely add new test cases. If it is a code base improvement, it typically means
        most of the cases are already covered. CAREFULLY consider if additional test cases for such improvement is needed.
...
1. Make intermediate commits to have your half-way result commited.
   ...
   - You should CAREFULLY consider, which intermediate commit should be checked, and which need not.
     - For example, when adding new test cases for a new feature, you should run the test first to make sure it fails as expected.
       Then, you should commit with `--no-verify`.
     - However, when finalizing a plan, you should always commit with pre-commit check.
2. This plan should be more detailed, and it is better to gather everything in this plan in advance:
   - DO NOT SAY: Find xxx in related files and make modifications.
   - MAKE A LIST: In file1, file2 and file3, we need to modify xxx to achieve yyy.
3. Read the [good code standard](./write-good-code.mdc) to make your own decision of refactoring good code without changing the semantics.
4. When finalizing your changes, always trigger a `pre-commit` test to make sure you modifications are good.
...
5. Leave a summary document in `dones/DONE-xxx.md`, where `xxx` briefly summarizes the title of the plan.
